...
func Part1() {
	...
	for _, row := range input {
		...
		if row == "" {
			count = count + countQuizQuestions(quiz)
...
func Part2() {
	...
	for _, row := range input {
		...
		if row == "" {
			count = count + countQuizQuestions2(quiz)
...
func countQuizQuestions2(quiz []string) int {

	answerMap := make(map[string]int)

	for _, quizLine := range quiz {

		if quizLine != "" {

			for _, letter := range quizLine {

				value, exists := answerMap[string(letter)]

				if !exists {
					answerMap[string(letter)] = 1
				}

				if exists {
					answerMap[string(letter)] = value + 1
				}
...
func countQuizQuestions(quiz []string) int {

	answerMap := make(map[string]bool)

	for _, quizLine := range quiz {
		fmt.Println(quizLine)
		if quizLine != "" {

			for _, letter := range quizLine {

				_, exists := answerMap[string(letter)]

				if !exists {
					answerMap[string(letter)] = true
				}
...
func parseInput(input string) []string {
	...
	splitByNewline := strings.Split(input, "\n")
