...
func calculateId(row int, col int) int {
	...
	return multipliedRow + col
}

func calculateRow(rowCode string) int {
	// numberOfRows := 128

	lower := 0
	upper := 127

	for idx, char := range rowCode {
		if char == 'F' {

			upper = getBetween(lower, upper)

			if idx == len(rowCode)-1 {
				return lower
			}
		}
		if char == 'B' {

			lower = getBetween(lower, upper) + 1
...
func calculateCol(colCode string) int {

	lower := 0
	upper := 7

	for idx, char := range colCode {
		if char == 'L' {

			upper = getBetween(lower, upper)

			if idx == len(colCode)-1 {
				return lower
			}
		}
		if char == 'R' {
...
func getBetween(lower int, upper int) int {
...
func parseInput() []string {
	...
	for i := 1; i < len(splitByNewline)-1; i++ {
