...
func Part1() {
	fmt.Println("hello from part 1 day 2")
	input := parseInput()
...
func getRange(rule string) []int {
	rangeString := strings.Split(rule, "-")

	var result []int

	lowerRange, _ := strconv.Atoi(rangeString[0])
	upperRange, _ := strconv.Atoi(rangeString[1])

	result = append(result, lowerRange)
	result = append(result, upperRange)
...
func parseInput() [][]string {
	...
	for i := 1; i < len(splitByNewline); i++ {
		...
		parsedInput = append(parsedInput, parsedRow)
	}
